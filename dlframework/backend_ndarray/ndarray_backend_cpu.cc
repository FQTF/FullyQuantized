...
void EwiseQuant(const AlignedArray& a, AlignedArray* out) {
  // scalar_t temp, temp_q;
  for (size_t i = 0; i < a.size; i++) {
    // temp = a.ptr[i];
    // temp = temp * decimal;
    // temp_q = (float)((int)(temp)) / decimal;
    // out->ptr[i] = temp_q;
    out->ptr[i] = round(a.ptr[i] * decimal) / decimal;
  }
}
...
void QMatmul(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m, uint32_t n,
            uint32_t p) {
  scalar_t temp, temp_q;
  for (size_t i=0; i<m; i++) {
    for (size_t j=0; j<p; j++) {
...
inline void AlignedDot(const float* __restrict__ a, 
  ...
  /**
   * Multiply together two TILE x TILE matrices
   *
   * Args:
