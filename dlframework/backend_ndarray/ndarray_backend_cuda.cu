...
__global__ void EwiseQuantKernel(const scalar_t* a, scalar_t* out, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    // scalar_t temp, temp_q;
    // temp = a[gid];
    // temp = temp * decimal;
    // temp_q = (scalar_t)((int)(temp)) / decimal;
    // out[gid] = temp_q;
    out[gid] = round(a[gid] * decimal) / decimal;
  }
}

void EwiseQuant(const CudaArray& a, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  EwiseQuantKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size);
}
...
__global__ void QMatmulKernel(scalar_t *a, scalar_t *b, scalar_t *out, size_t size, uint32_t M, 
                              uint32_t N, uint32_t P) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    size_t i = gid / P, j = gid % P;
    ...
    for (size_t k=0; k<N; k++) {
      ...
      // temp = temp * decimal;
      // temp_q = (scalar_t)((int)(temp)) / decimal;
      temp_q = round(temp * decimal) / decimal;
      sum += temp_q;
    }
    out[gid] = sum;
  }
...
void QMatmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
            uint32_t P) {
  /**
   * Multiply two (compact) matrices into an output (also comapct) matrix.  
  ...
  CudaDims dim = CudaOneDim(out->size);
  QMatmulKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, out->size, M, N, P);
