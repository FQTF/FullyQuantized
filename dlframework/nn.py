...
from dlframework.autograd import Tensor
from dlframework import ops
import dlframework.init as init
import numpy as np
...
class Linear(Module):
    def __init__(
        ...
        # NOTE: this line below will change type(self.bias) from 'Parameter' into 'dlfw.Tensor'!
...
class QLinear(Module):
    ...
    def forward(self, X: Tensor) -> Tensor:   
        w = ops.quant(self.weight)     
        X_mul_weight = ops.qmatmul(X, w)
        if self.bias:
            b = ops.quant(self.bias)
            return X_mul_weight + b.broadcast_to(X_mul_weight.shape)
        else:
            return X_mul_weight
...
class QConv(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.transpose((1, 2)).transpose((2, 3))
        w = ops.quant(self.weight)
        out = ops.qconv(x, w, stride=self.stride, padding=self.kernel_size//2)
        if self.bias:
            b = ops.quant(self.bias)
            out += b.reshape((1, 1, 1, self.out_channels)).broadcast_to(out.shape)
        # NOTE: out N H W C_out
        return out.transpose((2, 3)).transpose((1, 2))