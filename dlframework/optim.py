...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            ...
            # grad_data = dlfw.Tensor(param.grad.numpy(), device=param.device, dtype='float32').data \
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            if i not in self.m:
                # NOTE device
                self.m[i] = dlfw.init.zeros(*param.shape, device=param.device, dtype=param.dtype)
                self.v[i] = dlfw.init.zeros(*param.shape, device=param.device, dtype=param.dtype)
